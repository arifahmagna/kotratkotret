...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tbl_m_user WHERE id_tmu = ?"; // Menggunakan prepared statement untuk mencegah SQL Injection
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_sql = "UPDATE tbl_m_user SET name_tmu=?, email_tmu=?, role_tmu=? WHERE id_tmu=?";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param("sssi", $name, $email, $role, $id);  // Menggunakan prepared statement untuk mencegah SQL Injection
...
<body>
    ...
    <form method="POST" onsubmit="return confirmUpdate(event)">
        ...
        <input type="text" name="name" class="input" value="<?php echo htmlspecialchars($user['name_tmu']); ?>" required><br><br>

        <label class="label">Email:</label>
        <input type="email" name="email" class="input" value="<?php echo htmlspecialchars($user['email_tmu']); ?>" required><br><br>

        <label class="label">Role:</label>
        <select name="role" class="select">
            <option value="admin" <?php echo ($user['role_tmu'] == 'Admin') ? 'selected' : ''; ?>>Admin</option>
            <option value="cashiser" <?php echo ($user['role_tmu'] == 'Cashier') ? 'selected' : ''; ?>>Cashier</option>
        </select><br><br>
