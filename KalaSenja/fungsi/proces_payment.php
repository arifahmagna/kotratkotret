...
if (isset($_POST['checkout'])) {
    ...
    $cash_nominal = $_POST['cashNominal']; // Nominal uang yang dibayarkan
    $cash_nominal = str_replace('Rp. ', '', $cash_nominal); // Menghapus simbol Rp
    $cash_nominal = str_replace('.', '', $cash_nominal); // Menghapus titik untuk format ribuan
    $cash_nominal = (float) $cash_nominal;  // Pastikan menjadi float atau decimal
    ...
    try {
        ...
        $stmt->bind_param("sssiiis", $type_order, $status_order, $user_id, $payment_method_id, $total_amount, $cash_nominal, $customer_name);
        $stmt->execute();
        ...
        foreach ($_SESSION['cart'][$session_id] as $item) {
            ...
            if ($stmt_update_stock->affected_rows === 0) {
                throw new Exception("Stok tidak mencukupi untuk item menu dengan nama: " . $menu_id);
            }
        }
        ...
        // Mengecek role pengguna dan arahkan ke halaman yang sesuai
        if ($_SESSION['role'] == 'user') {
            echo "<script>alert('Checkout berhasil!'); window.location = '../costumer/cust_home.php';</script>";
        } else if ($_SESSION['role'] == 'cashier') {
            echo "<script>alert('Checkout berhasil!'); window.location = '../cashier/cashier_home.php';</script>";
        } else {
            // Jika role tidak dikenali
            echo "<script>alert('Role pengguna tidak dikenali.'); window.location = '../index.php';</script>";
        }
    } catch (Exception $e) {
